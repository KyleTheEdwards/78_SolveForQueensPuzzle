...
public class SolverForQueensPuzzle {
     ...
     */
    private void recordSolutionsStarted(BoardForQueensPuzzle currentBoard, int lastRankFilled) {
      nBoardsConsidered++;

        // Which has been requested, a base case or recursive case?
            // your code here
            // action(s) for base case(s)
      if(currentBoard.lastIsNg() == true){
      ...
      else if(lastRankFilled == boardSize - 1){
        /*System.out.println( "  for debugging: base case detected for..."
                        + System.lineSeparator()
                        + currentBoard 
                        + System.lineSeparator()
      ...
      // your code here
        ...
        BoardForQueensPuzzle boardToCheck = new BoardForQueensPuzzle(currentBoard);
        //System.out.println(boardToCheck);

        //boardToCheck.populate(0);
        //System.out.println(boardToCheck);

        for(int x = 0; x < boardSize; x++){
          boardToCheck = new BoardForQueensPuzzle(currentBoard);
          boardToCheck.populate(x);
          //System.out.println(boardToCheck);

          recordSolutionsStarted(boardToCheck, lastRankFilled + 1);
          //boardToCheck.depopulate();
        }
      }
    }
    ...
    public SolverForQueensPuzzle( int size) {
        ...
        boardSize = size;
        inProgress = new BoardForQueensPuzzle( boardSize);
        startAt = System.currentTimeMillis();
        recordSolutionsStarted(inProgress, -1); // "started" with an empty board
